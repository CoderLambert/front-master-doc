...
<div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '20px', margin: '20px 0' }}>
  ...
  <div>
    <h4>开发工具</h4>
    <ul>
      <li><strong>Create React App</strong> - 快速搭建开发环境</li>
      <li><strong>React Developer Tools</strong> - 浏览器调试工具</li>
      <li><strong>React Hot Loader</strong> - 热更新开发</li>
    </ul>
  </div>
  ...
  <div>
    <h4>UI库</h4>
    <ul>
      <li><strong>Material-UI</strong> - Material Design组件</li>
      <li><strong>Ant Design</strong> - 企业级UI组件</li>
      <li><strong>Chakra UI</strong> - 简单模块化的组件</li>
    </ul>
  </div>
  ...
  <div>
    <h4>状态管理</h4>
    <ul>
      <li><strong>Redux</strong> - 预测性状态管理</li>
      <li><strong>Zustand</strong> - 轻量级状态管理</li>
      <li><strong>React Query</strong> - 服务器状态管理</li>
    </ul>
  </div>
...
function Avatar({ name, src }) {
  return (
    <img
      ...
      onError={(e) => {
        e.target.src = \`https://ui-avatars.com/api/?name=\${encodeURIComponent(name)}&background=2196f3&color=fff&size=50\`;
      }}
    />
  );
}

function UserCard({ name, role, src, onUpdate, index }) {
  const [isEditing, setIsEditing] = React.useState(false);
  const [editName, setEditName] = React.useState(name);
  const [editRole, setEditRole] = React.useState(role);

  const handleUpdate = () => {
    onUpdate(index, editName, editRole);
    setIsEditing(false);
  };
    ...
    >
      ...
      <div style={{ flex: 1, marginLeft: '15px' }}>
        {isEditing ? (
          ...
          <div>
            <h3 style={{ margin: '0 0 5px 0', color: '#1565c0' }}>{editName}</h3>
            <p style={{ margin: 0, color: '#666' }}>{editRole}</p>
          </div>
        )}
      </div>
      {!isEditing && (
...
function App() {
  const [users, setUsers] = React.useState([
    { id: 1, name: 'Alice', role: '前端工程师', src: 'https://via.placeholder.com/50' },
    { id: 2, name: 'Bob', role: '产品经理', src: 'https://via.placeholder.com/50' },
    { id: 3, name: 'Charlie', role: '设计师', src: 'https://via.placeholder.com/50' }
  ]);

  const updateUser = (index, newName, newRole) => {
    const newUsers = [...users];
    newUsers[index] = { ...newUsers[index], name: newName, role: newRole };
    setUsers(newUsers);
  };

  return (
    <div style={{ padding: '20px' }}>
      <h1 style={{ color: '#1565c0', marginBottom: '20px' }}>👥 组件化用户列表</h1>
      <div style={{
        marginBottom: '20px',
        padding: '15px',
        ...
        <h4 style={{ margin: '0 0 10px 0', color: '#e65100' }}>💡 React组件组合</h4>
        <p style={{ margin: '5px 0', fontSize: '14px' }}>
          观察这个应用：<strong>App</strong>组件包含多个<strong>UserCard</strong>组件，
          而每个<strong>UserCard</strong>组件又包含<strong>Avatar</strong>组件。
        </p>
        <p style={{ margin: '5px 0', fontSize: '14px' }}>
          这就是React的<strong>组件化思想</strong>：小而美的组件可以组合成复杂的UI！
        </p>
      </div>
...
<Box style={{ padding: '15px', background: '#e8f5e9', borderRadius: '8px', margin: '20px 0' }}>
  <h4 style={{ margin: 0, color: '#2e7d32' }}>💡 组件化优势</h4>
  <p style={{ margin: '10px 0 0 0' }}>
    <strong>可复用</strong>：UserCard组件可以在多个地方使用<br/>
    <strong>可维护</strong>：每个组件职责单一，逻辑清晰<br/>
    <strong>可测试</strong>：可以独立测试每个组件<br/>
    <strong>可组合</strong>：多个组件组合成复杂应用
  </p>
</Box>
